...
- `/api/health` のヘルスチェック
- PDF / TXT / Markdown の資料登録 API
- PDF テキスト抽出、OCR 未処理判定、条文・見出し単位のチャンク化
...
- `ADMIN_UPLOAD_TOKEN`: 本番環境で資料登録APIを保護する管理用トークン
- `ADMIN_LOG_ACCESS_TOKEN`: Supabase Auth接続前に本番の個別質問ログ閲覧APIを閉じるための管理用トークン
- `ADMIN_INVITE_TOKEN`: 本番環境でユーザー招待APIを保護する管理用トークン
...
7. AuthenticationでEmail providerを有効化し、Site URLを `NEXT_PUBLIC_APP_URL`、Redirect URLに `/consent` を含める。
8. 自由サインアップを無効化し、管理者招待制で運用する。
9. 初期管理者をSupabase Authに作成し、`users`、`roles`、`user_roles`、`user_organizations` に所属とロールを付与する。
...
4. 本番環境では `ADMIN_UPLOAD_TOKEN` を設定する。
5. `/admin/documents` で資料名、資料種別、公開範囲、所管、タグ、PDF を選択し登録する。

登録後、Storage 保存、テキスト抽出、チャンク化、Embedding 生成、`document_chunks` 保存をサーバー側で実行します。テキスト抽出できない PDF は `ocr_required` になります。
...
削除は論理削除とし、`deleted_at`、`deleted_by`、`delete_reason` を保存します。資料は `document_versions` に旧版を保持し、案件は履歴と添付を残します。復元操作は管理者以上に限定し、`backup_restore_events` と `audit_logs` に記録します。
...
- Word、Excel、URL 登録は将来拡張用の設計対象で、Phase 1 では PDF、TXT、Markdown の実処理に対応しています。
