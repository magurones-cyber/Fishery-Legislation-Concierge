...
ブラウザで `http://localhost:3000/dashboard` を開きます。試行版はSupabase Auth、Supabase Database、Supabase Storageへの接続を前提にしています。
...
案件詳細の「実務向け文書生成」では、漁業者向け説明文、漁協向け指導メモ、庁内協議メモ、法令整理表、手続フロー、不足資料一覧、所管部署照会文案、補助金審査メモ等を生成し、画面上で編集できます。PDFはブラウザ印刷で出力し、Word出力は後続対応として扱います。
...
- Supabase Auth は接続済みです。利用者招待、同意状態、所属、ロールを確認して、資料登録、質問、案件作成、案件更新、案件削除を制御します。
...
- 管理画面は試行運用向けに、資料、カテゴリ、タグ、監査ログ、案件をSupabase実データから確認します。一部の高度機能、復元、Word出力、外部連携は運用手順と設計を先行しています。
