...
3. Productionでは `ADMIN_UPLOAD_TOKEN` を必ず設定する。未設定の場合、資料登録APIは503を返す。
...
6. Authentication > URL ConfigurationでSite URLを本番URLに設定し、Redirect URLに `/consent` を追加する。
...
1. 初期管理者をSupabase Authに作成する。
2. `users`、`user_organizations`、`user_roles` に初期管理者の所属とロールを登録する。
3. 管理者が `/admin/users` から利用者を招待する。
4. 利用者はメール・パスワード又はマジックリンクでログインする。
5. 初回ログイン時に `/consent` で利用規約、プライバシーポリシー、質問ログ分析に同意する。
6. 同意履歴は `user_consents` に保存する。
7. 同意しない利用者は質問機能を利用できない。
