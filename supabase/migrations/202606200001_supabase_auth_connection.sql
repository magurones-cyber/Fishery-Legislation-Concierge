-- Supabase Auth本接続: Cookieセッション、初回同意、所属・ロール判定のDB補助。
...
begin
  ...
  if auth_email is null then raise exception 'auth user not found'; end if;
  ...
  if target_role_id is null then raise exception 'role not found'; end if;
